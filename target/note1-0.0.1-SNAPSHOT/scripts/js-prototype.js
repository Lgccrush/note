...
var Hashtable = function (t) {
    ...
    function c() {
        var n = [], i = {}, u = {replaceDuplicateKey: !0, hashCode: e, equals: null}, o = arguments[0],
            a = arguments[1];
...
(function (k) {
    ...
    k._formatNumber = function (m, q, n, H, t) {
        var q = k.extend({}, k.fn.formatNumber.defaults, q);
        var F = e(q.locale.toLowerCase(), q.isFullLocale);
        var E = F.dec;
        var w = F.group;
        var l = F.neg;
        if (q.overrideGroupSep != null) {
            w = q.overrideGroupSep
        }
        if (q.overrideDecSep != null) {
            E = q.overrideDecSep
        }
        if (q.overrideNegSign != null) {
            l = q.overrideNegSign
        }
        var z = false;
        if (isNaN(m)) {
        ...
        if (q.isPercentage == true || (q.autoDetectPercentage && n.charAt(n.length - 1) == "%")) {
            m = m * 100
        }
        var B = "";
        if (q.format.indexOf(".") > -1) {
            ...
            if (q.round == true) {
                ...
                var L = m.toString();
                if (L.lastIndexOf(".") > 0) {
                    L = L.substring(0, L.lastIndexOf(".") + u.length + 1)
                }
                m = new Number(L)
            }
            var A = new Number(m.toString().substring(m.toString().indexOf(".")));
            decimalString = new String(A.toFixed(u.length));
            decimalString = decimalString.substring(decimalString.lastIndexOf(".") + 1);
            for (var I = 0; I < u.length; I++) {
                if (u.charAt(I) == "#" && decimalString.charAt(I) != "0") {
                    G += decimalString.charAt(I);
                    continue
                } else {
                    if (u.charAt(I) == "#" && decimalString.charAt(I) == "0") {
                        var r = decimalString.substring(I);
                        if (r.match("[1-9]")) {
                            G += decimalString.charAt(I);
                            continue
                        } else {
        ...
        if (q.format.indexOf(".") == -1) {
            ...
            D = q.format.substring(0, q.format.indexOf("."))
        }
        var K = "";
        if (!(v == 0 && D.substr(D.length - 1) == "#") || z) {
            var x = new String(Math.abs(v));
            var p = 9999;
            if (D.lastIndexOf(",") != -1) {
                p = D.length - D.lastIndexOf(",") - 1
            }
            var o = 0;
    ...
    k.parseNumber = function (u, z) {
        var z = k.extend({}, k.fn.parseNumber.defaults, z);
        var m = e(z.locale.toLowerCase(), z.isFullLocale);
        var r = m.dec;
        var x = m.group;
        var s = m.neg;
        if (z.overrideGroupSep != null) {
            x = z.overrideGroupSep
        }
        if (z.overrideDecSep != null) {
            r = z.overrideDecSep
        }
        if (z.overrideNegSign != null) {
            s = z.overrideNegSign
        }
        var l = "1234567890";
        var p = ".-";
        ...
        u = u.replace(r, ".").replace(s, "-");
        var y = "";
        var q = false;
        if (z.isPercentage == true || (z.autoDetectPercentage && u.charAt(u.length - 1) == "%")) {
            q = true
        }
        for (var v = 0; v < u.length; v++) {
            if (l.indexOf(u.charAt(v)) > -1) {
                y = y + u.charAt(v)
            } else {
                if (p.indexOf(u.charAt(v)) > -1) {
                    y = y + u.charAt(v);
                    p = p.replace(u.charAt(v), "")
                } else {
                    if (z.allowPostfix) {
                        break
                    } else {
